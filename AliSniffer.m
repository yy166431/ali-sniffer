...
//
// ALIWeChatSniffer_SwizzleFull.m
// Full sniffer: JS injection (v3) + AV AccessLog + safe delayed AV swizzle (initWithURL hooks)
// Intended for debugging/self-use. Install swizzles only after JS injection success to reduce crash risk.
//
// Build flags: -framework WebKit -framework AVFoundation -framework UIKit -framework Foundation
//
...
#endif
#if DEBUG_LOG
#define LOG(fmt, ...) NSLog((@"[ALI-SWIZZLE] " fmt), ##__VA_ARGS__)
#else
#define LOG(...)
#endif
...
static void install_av_swizzles_safe(void); // <--- swizzle install function
...
#pragma mark - inpage JS v3 (same as previous v3)
...
static void try_inject_when_ready(WKWebView *wv){
    ...
    [wv evaluateJavaScript:@"document.readyState" completionHandler:^(id val, NSError *err){
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kInjectDelay*NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            // 首次注入成功后 延迟安装 AV swizzles（仅安装一次）
            if (!g_av_swizzled){
                g_av_swizzled = YES;
                dispatch_async(gq, ^{ @try{ install_av_swizzles_safe(); }@catch(__unused NSException *e){} });
            }
        });
    }];
}
...
// Keep original function pointers
...
// Replacement implementations: capture URL then call original
static id replaced_AVURLAsset_initWithURL_options(id self, SEL _cmd, NSURL *URL, NSDictionary *options){
    ...
    // fallback: call super implementation via objc_msgSend if orig not set (very unlikely)
...
// Install swizzles safely (idempotent)
static void install_av_swizzles_safe(void){
    @try{
        if (g_av_swizzled) return; // already requested, but guard anyway
        ...
        // mark globally that we've installed
        g_av_swizzled = YES;
        // report installation
...
static void ALIWeChatSniffer_SwizzleFull_Init(void){
    if (!gq)    gq    = dispatch_queue_create("com.ali.wechat.swizzlefull", DISPATCH_QUEUE_SERIAL);
