//
//  AliSniffer.m — 最终成品（合并：auth_key 优先抓取）
//  说明：在你的基线文件上，仅增强了工具函数与 ReportURL 的优先级/去重策略；其他抓点与推送保持不变
//
...
// ====== 核心：ReportURL（合并了 auth_key 优先逻辑）======
...
#if SNIFFER_ENABLE_POPUP
            ShowPopupIfNeeded(@"已加载", @"已启用静默抓取 + 自动推送（含 auth_key 优先）");
#endif
        });
    }
}
