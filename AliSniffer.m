// AliSniffer.m — 静默版（无日志/无网络/无持久化）
// 仅在命中可播放 URL 时弹窗+复制；不向控制台输出。
// 覆盖点：NSURLSessionTask / NSURLProtocol(MIME|#EXTM3U) / AVPlayer(AVPlayerItem/AVURLAsset)
//         WKWebView(JS: fetch/XHR/video.src) / CFReadStream(fishhook) / (可选)libcurl
// 依赖：fishhook.h
...
#define SNIFFER_ENABLE_POPUP 1   // 需要弹窗就保留 1；想完全静默改为 0
#endif
...
// ====== 2) NSURLProtocol：识别 m3u8 / flv MIME 与 #EXTM3U ======
...
static void _sniffer_init(void) {
    @autoreleasepool {
        ...
        // NSURLProtocol in session config
        ...
        // libcurl（如果目标 App 没用，可留着不会报错）
        HookCurlIfPresent();

#if SNIFFER_ENABLE_POPUP
        ShowPopupIfNeeded(@"已加载", @"已启用静默抓取（m3u8/mp4/FLV/RTMP + 白名单）");
#endif
    }
}
